...
export default function AppointmentCalendar({ appointments, onViewDetails }) {
    ...
    const renderCalendarDays = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                <div key={day} className={`h-32 border-b border-r border-gray-100 p-2 transition-colors hover:bg-gray-50 ${isToday ? 'bg-purple-50/30' : ''}`}>
                    <div className="flex justify-between items-start mb-1">
                        <span className={`text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full ${isToday ? 'bg-purple-600 text-white' : 'text-gray-700'}`}>
                            {day}
                        </span>
