...
class ApiClient {
    ...
    setAuthTokens(token, refreshToken, remember = true) {
    ...
    async refreshAccessToken() {
        ...
        this.refreshPromise = (async () => {
            try {
                const response = await fetch(`${this.baseUrl}/api/${this.apiVersion}/Users/refresh-token`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ token: this.getToken() || '', refreshToken }),
                });
                const result = await this.parseResponseJson(response);
                if (!response.ok) return false;
                const auth = this.unwrapPayload(result);
                if (!auth?.token || !auth?.refreshToken) return false;

                const remember = localStorage.getItem('admin_refresh_token') !== null;
                this.setAuthTokens(auth.token, auth.refreshToken, remember);
                return true;
            } catch {
                return false;
            }
    ...
    async request(endpoint, options = {}) {
        ...
        const execute = async (retried = false) => {
            ...
            const isAuthRequest = /\/Users\/(login|refresh-token)$/i.test(normalizedEndpoint);

            if (response.status === 401 && !isAuthRequest && !retried) {
                if (await this.refreshAccessToken()) return execute(true);
    ...
    normalizeMediaContent(item) {
        ...
        return {
            ...
            mediaWidth: item.mediaWidth ?? item.MediaWidth ?? 0,
            mediaHeight: item.mediaHeight ?? item.MediaHeight ?? 0,
            mediaBytes: item.mediaBytes ?? item.MediaBytes ?? 0,
            mediaFormat: item.mediaFormat || item.MediaFormat || '',
        };
    }
    ...
    async uploadMediaContents(formData, memoryShort = false) {
        return this.upload(
            memoryShort ? '/api/v1/MediaContent/memory-shorts/uploads' : '/api/v1/MediaContent/uploads',
            formData,
            'POST'
        );
    }

    async getMemoryShorts() {
        const data = await this.request('/api/v1/MediaContent/memory-shorts');
        return this.normalizeArray(data, this.normalizeMediaContent);
    }

    async deleteMemoryShort(id) {
        return this.request(`/api/v1/MediaContent/memory-shorts/${id}`, { method: 'DELETE' });
    }

    async uploadImages(formData) {
        return this.upload('/api/v1/Images/uploads', formData, 'POST');
    }

    async getHeroSettings() {
        return this.request('/api/v1/site-settings/hero');
    }

    async updateHeroSettings(formData) {
        return this.upload('/api/v1/site-settings/hero', formData, 'PUT');
    }

    async resetHeroSettings() {
        return this.request('/api/v1/site-settings/hero/reset', { method: 'PUT' });
    }

    async getLivestreamSettings() {
        return this.request('/api/v1/site-settings/livestream');
    }
    ...
    async startYouTubeLive(title, description) {
        return this.request('/api/v1/site-settings/livestream/youtube/start', {
            method: 'POST',
            body: JSON.stringify({ title, description }),
        });
    }

    async stopYouTubeLive() {
        return this.request('/api/v1/site-settings/livestream/youtube/stop', { method: 'POST' });
    }
