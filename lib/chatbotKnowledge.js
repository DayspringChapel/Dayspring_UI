...
export const TOPICS = [
    ...
    {
        // Checked before "contact" so whatsapp-specific phrasing gets the
        // direct wa.me link instead of the generic appointment-page reply.
        id: 'whatsapp',
        test: (msg) => anyWord(msg, ['whatsapp', 'whats app']),
        answer: (opts) => {
            const digits = (opts?.whatsappNumber || '').replace(/[^\d]/g, '');
            if (!digits) {
                return "We don't have WhatsApp chat set up just yet — please reach us through our Appointment page instead: /appointment";
            }
            const text = encodeURIComponent("Hi DaySpring Chapel, I'd like to chat.");
            return `You can chat with us directly on WhatsApp: https://wa.me/${digits}?text=${text}`;
        },
    },
    {
        id: 'contact',
        test: (msg) => anyWord(msg, ['contact', 'phone number', 'call you', 'email', 'reach you']),
        answer:
